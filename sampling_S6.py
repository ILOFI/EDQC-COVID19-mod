...
def SEIR_MODEL1(Ecumm, Icumm, Rcumm, init_S, init_E, init_I, init_R, bmi, sigma, beta1 = 7.0):
    T = len(Ecumm)
    print(T)
#     case_data = Icumm
...
mod1 = SEIR_MODEL1(Ecumm, Icumm + Rcumm, Rcumm, N[streetid], initE[streetid], initI[streetid], initR[streetid], 
                  e2, sigma)
mc = MCMC(mod1)
mc.use_step_method(AdaptiveMetropolis, [mod1['alpha1'],])#, mod1['s0'], mod1['e0']
...
street_params = {}
street_params[streetname] = {'alpha_trace': mod1['alpha1'].trace(), 'beta_trace': np.array([7.0, ] * mod1['alpha1'].trace().shape[0])}
